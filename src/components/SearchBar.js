    ...
    return (
        <div className='flex items-center gap-2'>
            <div className='relative' ref={dropdownRef}>
                <div className='py-2 px-4 w-28 flex justify-between items-center border dark:text-gray-300 border-gray-500' onClick={handleShow}>
                    {usersPerPage}
                    {
                        isShow ? <i className='bx bx-chevron-down cursor-pointer'></i> : <i className='bx bx-chevron-up cursor-pointer'></i>
                    }
                </div>
                <div className={`absolute left-0 mt-2 w-28 flex flex-col items-center bg-white dark:bg-gray-700 border border-gray-300 rounded-lg shadow-lg z-10 px-4 py-2 space-y-3 ${isShow ? 'block' : 'hidden'}`}>
                    {
            ...
            <input
                type="text"
                placeholder="Search..."
                className="border border-gray-400 rounded px-4 py-2 mr-2 outline-none dark:bg-gray-600 dark:text-gray-200"
